...
trait CommonActions
{
    ...
    public function iCanSee($id)
    {
        ...
        Assert::assertTrue($element !== null, "Item '{$id}' did not appear on the page within 3 seconds");
    }
    ...
    public function iSelect($option)
    {
        ...
        Assert::assertNotEmpty($elements, 'Unable to find a select field');
    ...
    public function iSelectRadioButton($label)
    {
        ...
        Assert::assertNotNull($el, "Couldn't find a radio input with '$label'");
        $el->check();
    }
    ...
    public function iFillFormWith(TableNode $table)
    {
        ...
        foreach (GherkinHelper::convertTableToArrayOfData($table) as $field => $value) {
            ...
            Assert::assertNotEmpty($elements, "Unable to find '{$field}' field");
    ...
    public function formHasValue(TableNode $table)
    {
        ...
        foreach (GherkinHelper::convertTableToArrayOfData($table) as $field => $value) {
            $elements = $this->getXpath()->findFields($field);
            Assert::assertNotEmpty($elements, "Unable to find '{$field}' field");
            Assert::assertEquals($value, $elements[0]->getValue(), "Field values don't match");
        }
    }
    ...
    public function onPageSectionISeeLinks(TableNode $table, $pageSection = null)
    {
        ...
        foreach ($rows as $row) {
            ...
            Assert::assertNotEmpty($el, 'Unexpected link found');
        }
    }
    ...
    public function onPageSectionIDonTSeeLinks(TableNode $table, $pageSection = null)
    {
        ...
        foreach ($rows as $row) {
            ...
            Assert::assertEmpty($el, 'Unexpected link found');
        }
    }
    ...
    public function iSeeFollowingLinksIn(TableNode $table)
    {
        ...
        array_shift($rows);
        foreach ($rows as $row) {
            Assert::assertEquals(2, \count($row), 'The table should be have array with link and tag');
    ...
    public function iSeeTableWith(TableNode $table)
    {
        ...
        foreach ($rows as $row) {
            ...
            $message = "Couldn't find row with elements: '" . implode(',', array_merge([$mainColumn], $row)) . "'";
    ...
    public function iSeeWarning($message)
    {
        ...
        Assert::assertNotNull($el, "Couldn't find error/warning message '{$message}'");
    }
    ...
    public function onPageSectionISeeText($text, $pageSection = null)
    {
        ...
        Assert::assertNotNull($el, "Couldn't find '$text' text");
        Assert::assertEquals(trim($el->getText()), $text, "Couldn't find '$text' text");
    }
    ...
    public function isCheckedOption($label)
    {
        ...
        Assert::assertTrue($isChecked, "Checkbox $label is not checked");
    }
    ...
    public function isNotCheckedOption($label)
    {
        ...
        $isChecked = $this->getCheckboxChecked($label);
        Assert::assertFalse($isChecked, "Checkbox $label is checked");
    }
