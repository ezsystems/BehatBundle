...
# Run setup
docker-compose exec -T --user www-data app sh -c "bin/ezbehat --mode=standard --profile=setup --suite=multirepository"
docker-compose exec -T --user www-data app sh -c "composer run post-install-cmd"

# Reinstal database using the new repository
docker-compose exec -T --user www-data app sh -c "composer ezplatform-install"
