...
for i in "${prLinks[@]}"
do
  read -r -a VALUES <<< php bin/console ezsystems:github:get-pull-request-data $i $GITHUB_OAUTH_TOKEN
  ...
  composerDependencies+=("$composerDependencyString")
...
rm -rf ezplatform-page-builder
git clone --quiet http://github.com/mnocon/ezplatform-page-builder.git -b $TARGET_PAGE_BUILDER_BRANCH --quiet #TODO: Change to ezsystems
cd ezplatform-page-builder
git checkout -b $CONST_RANDOM_STRING --quiet || exit 1
git branch -D $TARGET_PAGE_BUILDER_BRANCH --quiet
